	...
	c.extend(c.validator, {
		...
		prototype: {
			init: function() {
				...
				c(this.currentForm).validateDelegate(":text, :password, :file, select, textarea,button",
					"focusin focusout keyup", a).validateDelegate(":radio, :checkbox, select, option,button", "click", a);
			...
			check: function(a) {
				...
				for(e in b) {
					...
					try {
						...
						if(a.type==='button')
						{
							g = c.validator.methods[e].call(this, this.findByName(a.name).text().replace(/\r/g, ""), a, f.parameters);
						}
						else 
						{
							g = c.validator.methods[e].call(this, a.value.replace(/\r/g, ""), a, f.parameters);
						}
//						var g = c.validator.methods[e].call(this, (a.value.replace(/\r/g, "") || this.findByName(a.name).text().replace(/\r/g, "")), a, f.parameters);
			...
			defaultShowErrors: function() {
				for(var a = 0; this.errorList[a]; a++) {
					 ...
					 * 新增提示语处理
					 */
					this.findByName(b.element.name).each(function() {
						var types = b.element.nodeName.toLowerCase();
						console.log(types);
						if(types != 'button') {
							if($(this).parent().find(types + '.error').length > 0) {
								if($(this).parent().find('label.error').length <= 0) {
									var d = c("<label/>").attr({
										"for": b.element.name,
										generated: true
									}).addClass('error').html(b.message || "");
									d.insertAfter($(this));
								}
							} else {
			...
			showLabel: function(a,
				...
				if(d.length) {
					...
					console.log(this.settings.errorElement);
			...
			getLength: function(a, b) {
				switch(b.nodeName.toLowerCase()) {
					case "button":
						return this.findByName(b.name).text().length;
					case "select":
