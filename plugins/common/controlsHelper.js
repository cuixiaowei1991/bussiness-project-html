(function(controlsHelper) {
	 ...
	 */
	controlsHelper.downBoxSelect = function(defaultText, dataList, elements) {
		$(elements).empty();
		$(elements).append("<option value=''>" + defaultText + "</option>");
		$.each(dataList, function(index, entry) {
			$(elements).append("<option value=" + entry.value + ">" + entry.name + "</option>");
		});
		//option改变事件
	...
	controlsHelper.readAsDataURLBase64 = function(result, file) {
		...
		reader.onload = function(e) {
			...
			//				var arry = this.result.split('base64,');
			//				if(arry.length > 1) {
			//					base64Str = arry[1];
			//				}
			//
